...
workdir <- 'D:/INCASI/Labtest/20171007/'
...
# import raw data
ecg <- read.csv(file.path(workdir,'20161007_Testdata_export_EXG.csv'), sep = '\t')

# import markers from ECG file ("events")
ecg_events <- read.csv(file.path(workdir,'20161007_Testdata_export_events.csv'), sep = '\t')
...
exg1_peaks <- read.csv(file.path(workdir,'EXG1_cropped_peaks_from_Kubios.txt'))
exg2_peaks <- read.csv(file.path(workdir,'EXG2_cropped_peaks_from_Kubios.txt'))
...
position <-  max(which(exg1_peaks < log_encode$clickonset_ecgtime_s[ipt])) # what's the time of the peak just before the click

log_encode$diff_click_peak_ms[ipt] <- (log_encode$clickonset_ecgtime_s[ipt] - exg1_peaks[position]) * 1000 
  
#  log_encode$clickonset_ecgtime_s[ipt] - exg1_peaks[position]
#  log_encode$clickonset_ecgtime_s[ipt] - exg1_peaks[position+1]
  
log_encode$frq_Hz[ipt] <- 60 / (exg1_peaks[position+1] - exg1_peaks[position])
...
H = circular(log_encode$rad,type="angle",units="radians",rotation="clock")
plot(H)
